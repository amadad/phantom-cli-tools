...
function drawSoftMotif(ctx: ReturnType<typeof createCanvas>['getContext'], width: number, height: number, brand: BrandFoundation, random: () => number): void {
...
function drawGridMotif(ctx: ReturnType<typeof createCanvas>['getContext'], width: number, height: number, brand: BrandFoundation, random: () => number): void {
...
export function generateSourceImage(options: SourceImageOptions): { imagePath: string; width: number; height: number; seed: number } {
  const width = 1600
  const height = 1600
  ...
  ctx.fillStyle = rgba(options.brand.visual.primary ?? options.brand.visual.palette.primary, 0.08)
