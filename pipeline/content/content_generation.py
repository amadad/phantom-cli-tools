...
from social.unified_social import post_to_platforms as unified_post_to_platforms
...
async def post_to_platforms(content: Dict[str, str], brand_config: Dict[str, Any], image_url: str = None, dry_run: bool = False) -> Dict[str, Dict]:
    """Post content to social platforms using unified social manager."""
    if dry_run:
    ...
    # Use actual unified social posting
    platforms = list(content.keys())
    media_paths = [image_url] if image_url else None
    
    # Convert content dict to string for unified posting
    content_text = "\n\n".join(content.values())
    
    return await unified_post_to_platforms(
