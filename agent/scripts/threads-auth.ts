/**
 * Threads OAuth 2.0 flow for getting Access Tokens
 * Run: npx tsx threads-auth.ts
 */
...
const REDIRECT_URI = process.env.THREADS_REDIRECT_URI || 'https://7e628944ad81.ngrok-free.app/callback'

// Threads scopes for publishing
...
async function getUserInfo(accessToken: string): Promise<{
  ...
  const response = await fetch(`https://graph.threads.net/v1.0/me?${params}`)
...
async function main() {
  if (!APP_ID || !APP_SECRET) {
    console.log('Missing THREADS_APP_ID or THREADS_APP_SECRET in .env')
    console.log('\nAdd these from your Facebook Developer App settings:')
    console.log('THREADS_APP_ID=your_threads_app_id')
    console.log('THREADS_APP_SECRET=your_threads_app_secret')
    return
  }
  ...
  console.log('2. Log in with your Threads account')
  console.log('3. Authorize the app')
  console.log('4. The access token will be displayed here\n')

  const server = createServer(async (req, res) => {
    const url = new URL(req.url!, 'http://localhost:3336')
    ...
    if (url.pathname === '/callback') {
      ...
      if (code) {
        ...
        try {
          ...
          console.log(`THREADS_BRAND_USER_ID="${userInfo.id}"`)
          console.log(`THREADS_BRAND_ACCESS_TOKEN="${longTokenData.access_token}"`)
          console.log('\n==============================\n')
        } catch (err) {
          console.error('\nError:', err)
        }
