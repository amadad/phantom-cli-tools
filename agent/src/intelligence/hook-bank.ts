 ...
 *
 * Usage:
 *   npx tsx hook-bank.ts givecare add --hook="..." --category=curiosity --multiplier=50
 *   npx tsx hook-bank.ts givecare list [--category=curiosity]
 *   npx tsx hook-bank.ts givecare search "caregiving"
 */
...
function getHookBankPath(brand: string): string {
  return join(__dirname, 'data', `${brand}-hooks.json`)
}
