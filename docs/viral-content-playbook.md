...
npx tsx src/intelligence/view-landscape.ts givecare

# Enrich with post-level data (for outlier detection)
npx tsx src/intelligence/enrich-apify.ts givecare --include-posts

# Detect outliers
npx tsx src/intelligence/detect-outliers.ts givecare

# Extract hooks from 50x+ content
npx tsx src/intelligence/extract-hooks.ts givecare --min-multiplier=50

# View hook bank
npx tsx src/intelligence/hook-bank.ts givecare --list
```
